...
write_report <- function(symb,d1,d2,a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,a12,P,RSI,CCI,BB,SMA,SMI,SAR, filename){
...
print(av1)
fprintf("\nAnalysis Report:\n",file=filename)
fprintf("\nStock: %s \nFrom: %s to %s\n",symb,d1,d2,file=filename,append = TRUE)
fprintf("\n*************************************************************************************\n",file=filename,append = TRUE)
fprintf("\nSummary: \n",file=filename,append = TRUE)
fprintf("\n ___________________________________________________________________________________________________________",file=filename,append = TRUE)
fprintf("\n| %10s | %20s | %20s | %10s | %10s | %20s |","Indicator","Magic # for buying","Magic # for selling","Buy sign","Sell sign","Max Return",file=filename,append = TRUE)
fprintf("\n ===========================================================================================================",file=filename,append = TRUE)
fprintf("\n| %10s | %20s | %20s | %10s | %10s | %20.2f |","RSI",av1[[1]][[3]],av1[[1]][[2]],if(av1[[2]]==1){"<"}else{">"},if(av1[[2]]==1){">"}else{"<"},av1[[1]][[1]],file=filename,append = TRUE)
fprintf("\n| %10s | %20s | %20s | %10s | %10s | %20.2f |","CCI",av2[[1]][[3]],av2[[1]][[2]],if(av2[[2]]==1){"<"}else{">"},if(av2[[2]]==1){">"}else{"<"},av2[[1]][[1]],file=filename,append = TRUE)
fprintf("\n| %10s | %20s | %20s | %10s | %10s | %20.2f |","BB",av3[[1]][[3]],av3[[1]][[2]],if(av3[[2]]==1){"<"}else{">"},if(av3[[2]]==1){">"}else{"<"},av3[[1]][[1]],file=filename,append = TRUE)
fprintf("\n| %10s | %20s | %20s | %10s | %10s | %20.2f |","SMA",av4[[1]][[3]],av4[[1]][[2]],if(av4[[2]]==1){"<"}else{">"},if(av4[[2]]==1){">"}else{"<"},av4[[1]][[1]],file=filename,append = TRUE)
fprintf("\n| %10s | %20s | %20s | %10s | %10s | %20.2f |","SMI",av5[[1]][[3]],av5[[1]][[2]],if(av5[[2]]==1){"<"}else{">"},if(av5[[2]]==1){">"}else{"<"},av5[[1]][[1]],file=filename,append = TRUE)
fprintf("\n| %10s | %20s | %20s | %10s | %10s | %20.2f |","SAR",av6[[1]][[3]],av6[[1]][[2]],if(av6[[2]]==1){"<"}else{">"},if(av6[[2]]==1){">"}else{"<"},av6[[1]][[1]],file=filename,append = TRUE)
fprintf("\n ***********************************************************************************************************\n",file=filename,append = TRUE)
...
print(round(P[length(P)] * (sum(BuyG[,2]) - sum(sellG[,2]))+sum(sellG[sellG[,2] == 1,4])-sum(BuyG[BuyG[,2] == 1,4]),2))
	...
	fprintf("\n\nCombining the magic numbers of indicators:\n",file=filename,append = TRUE)
        ...
        fprintf("\nRSI:T %s %s",if(av1[[2]]==1){"<"}else{">"},av1[[1]][[3]],file=filename,append = TRUE)
        fprintf("\nCCI:T %s %s",if(av2[[2]]==1){"<"}else{">"},av2[[1]][[3]],file=filename,append = TRUE)
        fprintf("\nBB:T %s %s",if(av3[[2]]==1){"<"}else{">"},av3[[1]][[3]],file=filename,append = TRUE)
        fprintf("\nSMA:T %s %s",if(av4[[2]]==1){"<"}else{">"},av4[[1]][[3]],file=filename,append = TRUE)
        fprintf("\nSMI:T %s %s",if(av5[[2]]==1){"<"}else{">"},av5[[1]][[3]],file=filename,append = TRUE)
        fprintf("\nSAR:T %s %s",if(av6[[2]]==1){"<"}else{">"},av6[[1]][[3]],file=filename,append = TRUE)
        fprintf("\nSell:",file=filename,append = TRUE)
        fprintf("\nRSI:T %s %s",if(av1[[2]]==1){">"}else{"<"},av1[[1]][[2]],file=filename,append = TRUE)
        fprintf("\nCCI:T %s %s",if(av2[[2]]==1){">"}else{"<"},av2[[1]][[2]],file=filename,append = TRUE)
        fprintf("\nBB:T %s %s",if(av3[[2]]==1){">"}else{"<"},av3[[1]][[2]],file=filename,append = TRUE)
        fprintf("\nSMA:T %s %s",if(av4[[2]]==1){">"}else{"<"},av4[[1]][[2]],file=filename,append = TRUE)
        fprintf("\nSMI:T %s %s",if(av5[[2]]==1){">"}else{"<"},av5[[1]][[2]],file=filename,append = TRUE)
        fprintf("\nSAR:T %s %s",if(av6[[2]]==1){">"}else{"<"},av6[[1]][[2]],file=filename,append = TRUE)
 	fprintf("\n*************************************************************************************",file=filename,append = TRUE)
	fprintf("\nBuy trades:\n",file=filename,append = TRUE)
        fprintf("\n ____________________________________________________________________________________",file=filename,append = TRUE)
	fprintf("\n| %10s |   %10s |  %10s |  %15s |  %20s |","Date","Stock Name","Buy","How many shares","Transaction Price",file=filename,append = TRUE)
	fprintf("\n ====================================================================================",file=filename,append = TRUE)
	fprintf("\n| %10s |   %10s |  %10s |  %15s |  %20.2f |",BuyG[BuyG[,2] == 1,3],symb,BuyG[BuyG[,2] == 1,1],"1",BuyG[BuyG[,2] == 1,4],file=filename,append = TRUE)
	fprintf("\n*************************************************************************************\n",file=filename,append = TRUE)
	fprintf("\nSell trades:\n",file=filename,append = TRUE)
        fprintf("\n ____________________________________________________________________________________",file=filename,append = TRUE)
	fprintf("\n| %10s |   %10s |  %10s |  %15s |  %20s |","Date","Stock Name","Buy","How many shares","Transaction Price",file=filename,append = TRUE)
	fprintf("\n ====================================================================================",file=filename,append = TRUE)
	fprintf("\n| %10s |   %10s |  %10s |  %15s |  %20.2f |",sellG[sellG[,2] == 1,3],symb,sellG[sellG[,2] == 1,1],"1",sellG[sellG[,2] == 1,4],file=filename,append = TRUE)
	fprintf("\n*************************************************************************************\n",file=filename,append = TRUE)
        fprintf("\ntotal Buy = %6.2f",sum(BuyG[BuyG[,2] == 1,4]),file=filename,append = TRUE)
        fprintf("\ntotal Sell = %6.2f",sum(sellG[sellG[,2] == 1,4]),file=filename,append = TRUE)
        fprintf("\nrealized Earning = %6.2f",round(P[length(P)] * (sum(BuyG[,2]) - sum(sellG[,2]))+sum(sellG[sellG[,2] == 1,4])-sum(BuyG[BuyG[,2] == 1,4]),2),file=filename,append = TRUE)
