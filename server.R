...
getQuote_ <- function(ticks) {
  qRoot <-
    "https://query1.finance.yahoo.com/v7/finance/quote?fields=symbol,longName,regularMarketTime,regularMarketPreviousClose,regularMarketChange,regularMarketOpen,regularMarketDayHigh,regularMarketDayLow,regularMarketVolume&formatted=false&symbols="
  z <- fromJSON(paste(qRoot, paste(ticks, collapse = ","), sep = ""))
  z <-
    z$quoteResponse$result[, c(
      "symbol",
      "regularMarketTime",
  ...
  z$symbol <- NULL
  names(z) <-
    c("Time", "Last", "Change", "Open", "High", "Low", "Volume")
...
shinyServer(function(input, output) {
  ...
  observe({
    if (is.null(mode$data)) {
      ...
      showElement("trade")
      hideElement("dateG")
      hideElement("analyzeG")
      hideElement("playG")
      hideElement("indicatorG")
      hideElement("strategyG")
      showElement("tt1")
      showElement("tt2")
      showElement("tt3")
      showElement("tt4")
      showElement("table2")
      hideElement("nextb")
      hideElement("checkGroup")
      hideElement("checkG")
    } else{
      showElement("analyzeG")
      showElement("playG")
      showElement("dateG")
      showElement("indicatorG")
      showElement("strategyG")
      hideElement("dates")
      hideElement("trade")
  ...
  observe({
    if ((wselectbox() == 1 &
         is.null(mode$data)) |
        (wselectbox() == 3 & w$data != 1 & is.null(mode$data))) {
      showElement("text15")
      showElement("text16")
      showElement("text17")
      showElement("text18")
  ...
  output$text17 <- renderText({
    if (w$data == 1 | wselectbox() == 1) {
      paste("# of buy = ", resulttrade2()[[4]])
    } else{
      paste("# of buy = ", length(bu1()[, 1]))
    }
  })
  output$text18 <- renderText({
    if (w$data == 1 | wselectbox() == 1) {
      paste(" # of sell = ", resulttrade2()[[5]])
    } else{
      paste(" # of sell = ", length(se1()[, 1]))
    }
  })
  output$text19 <- renderText({
    if (w$data == 1 | wselectbox() == 1) {
      paste("total Buy =", round(resulttrade2()[[6]], 2))
    } else{
      paste("total Buy =", round(sum(bu1()[, 2]), 2))
    }
  })
  output$text20 <- renderText({
    if (w$data == 1 | wselectbox() == 1) {
      paste("total Sell = ", round(resulttrade2()[[7]], 2))
    } else{
      paste("total Sell =", round(sum(se1()[, 2]), 2))
    }
  })
  output$text21 <- renderText({
    if (w$data == 1 | wselectbox() == 1) {
      paste("How many stocks are unrealized? ",
            resulttrade2()[[4]] - resulttrade2()[[5]])
    } else{
      paste("How many stocks are unrealized? ",
            length(bu1()[, 1]) - length(se1()[, 1]))
    }
  })
  output$text21j <- renderText({
    if (w$data == 1 | wselectbox() == 1) {
      paste(
        "What is the earning? ",
        round(
          dataInput5()[length(dataInput5()[, 4]), 4] * (resulttrade2()[[4]] - resulttrade2()[[5]]) +
            resulttrade2()[[7]] - resulttrade2()[[6]],
          2
        )
      )
    } else{
      paste("What is the earning? ",
            round(
              dataInput5()[length(dataInput5()[, 4]), 4] * (length(bu1()[, 1]) - length(se1()[, 1])) +
                sum(se1()[, 2]) - sum(bu1()[, 2]),
              2
            ))
    }
  ...
  observe({
    if (wselectbox() == 2 & is.null(mode$data)) {
      showElement("RSI")
      showElement("RSI1")
      showElement("RSI0")
      showElement("RSI10")
      ...
      showElement("rsivn10")
      showElement("rsivn11")
      showElement("rsivn110")
      
      showElement("rsivn2")
      showElement("rsivn20")
      showElement("rsivn12")
      showElement("rsivn120")
      
      showElement("rsivn3")
      showElement("rsivn30")
      showElement("rsivn13")
      showElement("rsivn130")
      ...
      hideElement("rsivn10")
      hideElement("rsivn11")
      hideElement("rsivn110")
      
      hideElement("rsivn2")
      hideElement("rsivn20")
      hideElement("rsivn12")
      hideElement("rsivn120")
      
      hideElement("rsivn3")
      hideElement("rsivn30")
      hideElement("rsivn13")
      hideElement("rsivn130")
  ...
  observe({
    if ((
      is.null(indicator2$inp1) &
      is.null(indicator2$inp2) &
      is.null(indicator2$inp3) &
      is.null(indicator2$inp4) &
      is.null(indicator2$inp5) &
      is.null(indicator2$inp6) &
      is.null(indicator2$inp7) &
      is.null(indicator2$inp8) &
      is.null(indicator2$inp9) &
      is.null(indicator2$inp10) &
      is.null(indicator2$inp11) &
      is.null(indicator2$inp12) &
      !is.null(mode$data)
    ) | wselectbox() != 2 | !is.null(mode$data)) {
      hideElement("text9")
      hideElement("text9m")
      hideElement("text10")
      hideElement("text11")
  ...
  resultbuy <- eventReactive(input$Buy, {
    if ((
      is.null(indicator$inp1) &
      is.null(indicator$inp2) &
      is.null(indicator$inp3) &
      is.null(indicator$inp4) &
      is.null(indicator$inp5) &
      is.null(indicator$inp6) &
      is.null(indicator$inp7) &
      is.null(indicator$inp8) &
      is.null(indicator$inp9) &
      is.null(indicator$inp10) &
      is.null(indicator$inp11) & is.null(indicator$inp12)
    )) {
      0
    } else{
      tradeBuy(
  ...
  resultsell <- eventReactive(input$Sell, {
    if ((
      ...
      is.null(indicator2$inp9) &
      is.null(indicator2$inp10) &
      is.null(indicator2$inp11) & is.null(indicator2$inp12)
    )) {
      0
    } else{
      tradeBuy(
  ...
  output$te1 <- renderText({
    if (!is.null(conf$data2)) {
      paste("Are you sure with this strategy:")
    } else{
      paste("")
    }
  })
  output$tek1 <- renderText({
    if (!is.null(conf$data2)) {
      paste("Buy:")
    } else{
      paste("")
  ...
  output$te4n <- renderText({
    if (!is.null(conf$data2)) {
      paste("BB:T-1", input$BB, if (indicator$inp3 == "") {
        "not filled"
      } else{
        indicator$inp3
      }, "-T", input$BB0, if (indicator$inp6 == "") {
        "not filled"
      } else{
        indicator$inp6
      })
  ...
  output$te6 <- renderText({
    if (!is.null(conf$data2)) {
      paste("___________________________________")
    } else{
      paste("")
    }
  })
  output$text9 <- renderText({
    if (!is.null(conf$data)) {
      paste("Date:    ", input$dates[1], "   to    ", input$dates[2])
    }
  })
  output$text9m <- renderText({
    if (!is.null(conf$data)) {
      paste("Stock:  ", input$symb)
    } else{
      paste("")
  ...
  output$text14p <- renderText({
    if (!is.null(conf$data)) {
      paste("How many stocks are unrealized? ",
            sum(resultbuy()[, 2]) - sum(resultsell()[, 2]))
    }
  })
  output$text14 <- renderText({
    if (!is.null(conf$data)) {
      paste("what is earning? ", round(
  ...
  output$Report <- downloadHandler(
    filename = function() {
      paste(input$symb, "-indicator-report-", Sys.Date(), ".txt", sep = "")
    },
    content = function(file) {
      withProgress(message = "Generating indicator report. Please wait.",
      indicator_report(
        symb = input$symb,
        RSI = input$RSI,
        ind1_inp1 = indicator$inp1,
        RSI0 = input$RSI0,
        ind1_inp4 = indicator$inp4,
        CCI = input$CCI,
        ind1_inp2 = indicator$inp2,
        CCI0 = input$CCI0,
        ind1_inp5 = indicator$inp5,
        BB = input$BB,
        ind1_inp3 = indicator$inp3,
        BB0 = input$BB0,
        ind1_inp6 = indicator$inp6,
        SMA = input$SMA,
        ind1_inp7 = indicator$inp7,
        SMA0 = input$SMA0,
        ind1_inp10 = indicator$inp10,
        SMI = input$SMI,
        ind1_inp8 = indicator$inp8,
        SMI0 = input$SMI0,
        ind1_inp11 = indicator$inp11,
        SAR = input$SAR,
        ind1_inp9 = indicator$inp9,
        SAR0 = input$SAR0,
        ind1_inp12 = indicator$inp12,
        RSI1 = input$RSI1,
        ind2_inp1 = indicator2$inp1,
        RSI10 = input$RSI10,
        ind2_inp4 = indicator2$inp4,
        CCI1 = input$CCI1,
        ind2_inp2 = indicator2$inp2,
        CCI10 = input$CCI10,
        ind2_inp5 = indicator2$inp5,
        BB1 = input$BB1,
        ind2_inp3 = indicator2$inp3,
        BB10 = input$BB10,
        ind2_inp6 = indicator2$inp6,
        SMA1 = input$SMA1,
        ind2_inp7 = indicator2$inp7,
        SMA10 = input$SMA10,
        ind2_inp10 = indicator2$inp10,
        SMI1 = input$SMI1,
        ind2_inp8 = indicator2$inp8,
        SMI10 = input$SMI10,
        ind2_inp11 = indicator2$inp11,
        SAR1 = input$SAR1,
        ind2_inp9 = indicator2$inp9,
        SAR10 = input$SAR10,
        ind2_inp12 = indicator2$inp12,
        dataInput5 = dataInput5,
        resultbuy = resultbuy,
        resultsell = resultsell,
        filename = file
      ))
    }
  )
  ...
  p1 <- reactive({
    round(min(dataInputc()[, 1]), 0)
  })
  p2 <- reactive({
    round(max(dataInputc()[, 1]), 0)
  })
  ...
  buyd <-
    reactive({
      if (w$data == 1) {
        xts(buydt()[, 3], order.by = as.POSIXct(as.Date(buydt()[, 1])))
      } else{
        xts(buydt()[, 2], order.by = as.POSIXct(as.Date(buydt()[, 1])))
      }
    })
  selld <-
    reactive({
      if (w$data == 1) {
        xts(selldt()[, 3], order.by = as.POSIXct(as.Date(selldt()[, 1])))
      } else{
        xts(selldt()[, 2], order.by = as.POSIXct(as.Date(selldt()[, 1])))
      }
    })
  PC <- reactive({
    sort(rbind(buyd(), selld()))
  })
  RSindex0 <- reactive({
    RSI(dataInput()[, 4], winsz$fastsz)
  })
  CCindex0 <-
    reactive({
      CCI(dataInput()[, 2:4], n = winsz$data, c = 0.015)
    })
  RSindex5 <- reactive({
    RSI(dataInput5()[, 4], winsz$fastsz)
  })
  CCindex5 <-
    reactive({
      CCI(dataInput5()[, 2:4], n = winsz$data, c = 0.015)
    })
  BBandsc5 <-
    reactive({
      BBands(dataInput5()[, 2:4], n = winsz$bbnd, sd = 2)
    })
  MACDc <-
    reactive({
      MACD(
  ...
  BBandsc0 <-
    reactive({
      BBands(dataInput()[, 2:4], n = winsz$bbnd, sd = 2)
    })
  sma0 <-
    reactive({
      SMA(dataInput()[, 4], round(winsz$fastsz + winsz$slowsz / 2))
    })
  sma5 <-
    reactive({
      SMA(dataInput5()[, 4], round(winsz$fastsz + winsz$slowsz / 2))
    })
  sar0 <- reactive({
    SAR(dataInput()[, 2:3])
  })
  ...
  RSindex <- reactive({
    if (is.null(mode$data)) {
      same_time1(RSindex0(), dataInputc()[, 4])
    } else{
      RSindex0()
    }
  })
  CCindex <- reactive({
    if (is.null(mode$data)) {
      same_time1(CCindex0(), dataInputc()[, 4])
    } else{
      CCindex0()
    }
  })
  BBandsc <- reactive({
    if (is.null(mode$data)) {
      same_time(BBandsc0()[, 2], dataInputc()[, 4], BBandsc0())
    } else{
      BBandsc0()
    }
  })
  ...
  smi1 <- reactive({
    if (is.null(mode$data)) {
      same_time1(smi0()[, 1], dataInputc()[, 4])
    } else{
      smi0()
    }
  })
  ...
  indimp_G <-
    reactive({
      if (selboxindG() == 1) {
        RSindex()
      } else if (selboxindG() == 2) {
        CCindex()
      } else if (selboxindG() == 3) {
        BBandsc()[, 2]
      }
      else if (selboxindG() == 4) {
        sma1()
      } else if (selboxindG() == 5) {
        smi1()[, 1]
      } else if (selboxindG() == 6) {
        sar1()
      }
    })
  optimalret <-
    reactive({
      analysis_strategy(indimp_G(), dataInput()[, 4], analsign$d)
    })
  op <-
    reactiveValues(
      a1 = NULL,
  ...
  output$downloadReport <- downloadHandler(
    ...
    content = function(file) {
      withProgress(message = "Generating analysis report. Please wait.",
      write_report(
        input$symb,
        input$dateG[1],
        input$dateG[2],
  ...
  output$txgame1 <- renderText({
    if (!is.null(oparray$data)) {
      paste(time(oparray$data[1]))
    } else{
      paste("")
    }
  })
  output$txgame2 <-
    renderText({
      if (!is.null(oparray$data) & length(oparray$data) != 1) {
        paste(time(oparray$data[2]))
      } else{
        paste("")
      }
    })
  output$txgame3 <- renderText({
    if (!is.null(oparray$data)) {
      paste(round(sum(oparray$data2) - sum(oparray$data), 2))
    } else{
      paste("")
    }
  })
  ...
  observeEvent(input$nextb, {
    if (input$checkGroup == 1) {
      if (is.null(oparray$data)) {
        oparray$data <-
          dataInput6()[length(dataInput6()[, 4]), 4]
      } else if (sum(time(oparray$data) == time(dataInput6()[length(dataInput6()[, 4]), 4])) == 0) {
        oparray$data <-
          c(oparray$data, dataInput6()[length(dataInput6()[, 4]), 4])
      }
      ...
      if (!is.null(oparray$data)) {
        if (is.null(oparray$data2)) {
          oparray$data2 <-
            dataInput6()[length(dataInput6()[, 4]), 4]
        } else if (sum(time(oparray$data2) == time(dataInput6()[length(dataInput6()[, 4]), 4])) == 0 &
                   length(oparray$data) > length(oparray$data2)) {
          oparray$data2 <-
            c(oparray$data2, dataInput6()[length(dataInput6()[, 4]), 4])
        }
      }
    }
  ...
  output$dygraph <-
    renderDygraph({
      if (is.null(mode$data) &
          v$data == 1 & !isempty(buyd()) & !isempty(selld())) {
        dygraph(cbind(PC(), buyd(), selld(), BBandsc()[, 1:3]),
                main = "Stock Prices",
                ylab = "Prices") %>%
          dySeries(
            "PC..",
      ...
      } else if (is.null(mode$data) &
                 v$data == 1 & isempty(selld()) & !isempty(buyd())) {
        dygraph(cbind(PC(), buyd(), BBandsc()[, 1:3]),
                main = "Stock Prices",
                ylab = "Prices") %>%
          dySeries("..1",
                   label = "Price",
        ...
        #   dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1"))
                 ...
                 v$data == 1) {
        dygraph(cbind(PC(), BBandsc()[, 1:3]), main = "Stock Prices", ylab = "Prices") %>%
          dySeries("..1",
                   label = "Price",
                   color = "magenta",
                   strokeWidth = 2) %>%
        ...
        dygraph(L24HOUR(), ylab = "Price") %>%
          dySeries("V1", label = "Price", color = "darkgreen")
                 ...
                 !is.null(oparray$data3) &
                 !is.null(oparray$data4)) {
        dygraph(
          cbind(
            dataview(),
            oparray$data3,
          ...
          dyOptions(
            ...
            pointSize = 5
                 ...
                 !is.null(oparray$data3)) {
        dygraph(
          cbind(dataview(), oparray$data3, dataInput6()[length(dataInput6()[, 4]), 4], dataviewBB()[, 1:3]),
          main = "Game Mode",
          ylab = "Price"
        ) %>%
          dySeries(
          ...
          dyOptions(
            ...
            pointSize = 5
                 ...
                 !is.null(oparray$data4)) {
        dygraph(
          cbind(dataview(), oparray$data4, dataInput6()[length(dataInput6()[, 4]), 4], dataviewBB()[, 1:3]),
          main = "Game Mode",
          ylab = "Price"
        ) %>%
          dySeries(
        ...
        dygraph(
          cbind(dataview(), dataInput6()[length(dataInput6()[, 4]), 4], dataviewBB()[, 1:3]),
          main = "Game Mode",
          ylab = "Price"
        ) %>%
          dySeries(
  ...
  output$dygraph2 <-
    renderDygraph({
      if (is.null(mode$data)) {
        dygraph(RSindex(), ylab = "RSI") %>%
          dySeries("V1", label = "RSI", color = "darkgreen")
      } else{
  ...
  output$dygraph3 <-
    renderDygraph({
      if (is.null(mode$data)) {
        dygraph(dataInputc()[, 5], ylab = "Volume") %>%
  ...
  output$dygraph4 <-
    renderDygraph({
      if (is.null(mode$data)) {
        dygraph(CCindex(), ylab = "CCI") %>%
          dySeries("V1", label = "CCI", color = "darkgreen")
      } else{
  ...
  output$dygraph5 <-
    renderDygraph({
      if (is.null(mode$data)) {
        dygraph(sma1(), ylab = "SMA") %>%
          dySeries("V1", label = "SMA", color = "darkgreen")
      } else{
  ...
  output$dygraph6 <-
    renderDygraph({
      if (is.null(mode$data)) {
        dygraph(smi1()[, 1], ylab = "SMI") %>%
          dySeries("V1", label = "SMI", color = "darkgreen")
      } else{
  ...
  output$dygraph7 <-
    renderDygraph({
      if (is.null(mode$data)) {
        dygraph(sar1(), ylab = "SAR") %>%
          dySeries("V1", label = "SAR", color = "darkgreen")
      } else{
